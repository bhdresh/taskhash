...
FileDelete("_processes_"&@ComputerName)
Global $file = FileOpen("_processes_"&@ComputerName, 1)
...
EndIf
    ...
    For $i = 1 To $aProcessList[0][0]
	   ;MsgBox(0, "", $aProcessList[$i][1])
    ;MsgBox($MB_SYSTEMMODAL, "", $aProcessList[$i][0] & @CRLF & "PID: " & $aProcessList[$i][1])
	$avRET = _ProcessListProperties($aProcessList[$i][1])

   if not $avRET[0][0] = 0 Then
   	$path1 = $avRET[1][5]
   	  ;_ArrayDisplay($avRET, "test")
   	 ;MsgBox(0, "", $path1)
	if FileExists($path1) Then
	 Local $dHash = _Crypt_HashFile($path1, $CALG_MD5)
	;ConsoleWrite($path1 & " --- " & $dHash & @CRLF)
	FileWrite($file, $path1 & " --- " & $dHash & @LF)
 EndIf
 EndIf
	Next
FileClose($file)
